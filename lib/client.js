...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var balanceCache = { promise: null, value: null, error: "" };
    function queryBalance(force) {
      if (force || !balanceCache.promise) {
    ...
    var costSummaryDefinition = {
      ...
      start: function (_context, match) {
        ...
        return { turn: match.event.data.turn, steps: {} };
      },
      ...
      buildViewNode: function (context) {
        ...
        return chatNode(context, "cost-summary", anchor + 0.15, { turn: state.turn, calls: calls });
      }
    };
    ...
    function CostSummaryRow(props) {
      ...
      useEffect(function () {
        ...
        queryBalance(false).then(function (b) { if (alive) setBal(b); });
        var listener = function () {
          queryBalance(true).then(function (b) { if (alive) setBal(b); });
        };
        refreshListeners.push(listener);
        ensureRefreshTimer();
        return function () {
          alive = false;
          var i = refreshListeners.indexOf(listener);
      ...
      if (bal && bal.value && bal.value.infos && bal.value.infos.length) {
        var b0 = bal.value.infos[0];
