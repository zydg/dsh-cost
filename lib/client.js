  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        modelsTitle: "模型与价格",
        modelsHint: "手动添加/删除模型并编辑高峰、空闲单价，保存后按每次调用时间逐次计价。价格以人民币存储，单位为「元 / 百万 tokens」。",
        weekendLabel: "周六/周日全天按低谷价",
        addModel: "＋ 添加模型",
        savePrices: "保存价格",
        resetPrices: "恢复官方默认",
        saving: "保存中…",
        ...
        rateOut: "输出",
        removeModel: "删除",
        modelKeyRequired: "模型名不能为空",
        modelKeyDup: "模型名重复",
        emptyModels: "至少保留一个模型",
        rateUnit: "元 / 百万 tokens",
        footerRefreshed: "统计行会用新价格重新渲染。",
        peakWindowsTitle: "高峰时段（北京时间）",
        peakWindowsHint: "格式 HH:MM，开始须早于结束；可添加多段，全部删除表示全天低谷。",
        addWindow: "＋ 添加时段",
        windowInvalid: "时段不合法：请填 HH:MM，且开始早于结束"
      },
      en: {
        round: "Round #",
        input: "Input",
        ...
        modelsHint: "Add or remove models and edit their peak / off-peak rates. Each call is priced by its own timestamp after saving. Rates are stored in CNY (yuan per 1M tokens).",
        weekendLabel: "Saturday/Sunday all off-peak",
        addModel: "+ Add model",
        savePrices: "Save prices",
        resetPrices: "Reset to official",
        saving: "Saving…",
        savedPrices: "Prices saved",
        loadingPrices: "Loading…",
        priceError: "Save failed",
        colKey: "Model id",
        colLabel: "Label",
        colPeak: "Peak",
        colOffPeak: "Off-peak",
        ...
        rateOut: "Output",
        removeModel: "Remove",
        modelKeyRequired: "Model id is required",
        modelKeyDup: "Duplicate model id",
        emptyModels: "Keep at least one model",
        rateUnit: "CNY / 1M tokens",
        footerRefreshed: "Footer lines re-render with the new prices.",
        peakWindowsTitle: "Peak windows (Beijing time)",
        peakWindowsHint: "Use HH:MM with start before end; add multiple windows, remove all for all-day off-peak.",
        addWindow: "+ Add window",
        windowInvalid: "Invalid window: use HH:MM with start before end"
      }
    };
    ...
    var pricingValue = null;
    var pricingListeners = [];

    // 价格表广播：设置页保存/恢复后，已挂载的统计行无需刷新即可用新价重算。
    function subscribePricing(fn) {
      pricingListeners.push(fn);
      return function () {
        var i = pricingListeners.indexOf(fn);
        if (i >= 0) pricingListeners.splice(i, 1);
      };
    }

    function setPricingValue(next) {
      pricingValue = next;
      for (var i = 0; i < pricingListeners.length; i++) {
        try { pricingListeners[i](next); } catch (e) {}
      }
      return next;
    }

    // 保存价格后把宿主返回的权威价格表写入缓存并广播。
    function applyPricingResult(p) {
      if (!p) return;
      pricingPromise = Promise.resolve(p);
      setPricingValue(p);
    }
    ...
    function getPricing() {
      if (!pricingPromise) {
        pricingPromise = api({ action: "list" }).then(function (res) {
          ...
          var p = res && res.ok && res.pricing ? res.pricing : DEFAULT_PRICING;
          setPricingValue(p);
          return p;
        }).catch(function () { return pricingValue || DEFAULT_PRICING; });
      }
    ...
    function isPeak(ts, peakWindows, weekendOffPeak) {
      // 数组（含空数组 []）优先：[] 表示全天低谷。
      var windows = Array.isArray(peakWindows) ? peakWindows : DEFAULT_PRICING.peakWindows;
    ...
    function CostSummaryRow(props) {
      ...
      var [pricing, setPricingState] = useState(pricingValue);
      ...
      useEffect(function () {
        var alive = true;
        getPricing().then(function (p) { if (alive) setPricingState(p); });
        var unsubscribePricing = subscribePricing(function (p) { setPricingState(p); });
        // Fresh balance query on every row mount (= every completed turn).
        queryBalance().then(function (b) { if (alive) setBal(b); });
        return function () { alive = false; unsubscribePricing(); };
      }, []);
    ...
    // ── model & price editor (manual pricing) ────────────────────────────────
    var stPrice = {
      box: { border: "1px solid rgba(128,128,128,.25)", borderRadius: 8, padding: "10px 12px", display: "flex", flexDirection: "column", gap: 10, maxWidth: 680, boxSizing: "border-box" },
      model: { border: "1px solid rgba(128,128,128,.2)", borderRadius: 6, padding: "8px 10px", display: "flex", flexDirection: "column", gap: 6 },
      line: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" },
      grid: { display: "grid", gridTemplateColumns: "64px 1fr 1fr 1fr", gap: "4px 8px", alignItems: "center", fontSize: 11.5 },
      th: { opacity: 0.6, fontSize: 11 },
      keyInput: { border: "1px solid rgba(128,128,128,.35)", background: "transparent", borderRadius: 6, padding: "4px 8px", fontSize: 12, color: "inherit", width: 168, fontFamily: "inherit" },
      labelInput: { border: "1px solid rgba(128,128,128,.35)", background: "transparent", borderRadius: 6, padding: "4px 8px", fontSize: 12, color: "inherit", flex: 1, minWidth: 120, fontFamily: "inherit" },
      num: { border: "1px solid rgba(128,128,128,.35)", background: "transparent", borderRadius: 6, padding: "3px 6px", fontSize: 12, color: "inherit", width: "100%", boxSizing: "border-box", textAlign: "right" },
      time: { border: "1px solid rgba(128,128,128,.35)", background: "transparent", borderRadius: 6, padding: "3px 6px", fontSize: 12, color: "inherit", width: 86 },
      del: { border: "1px solid rgba(232,84,84,.4)", background: "transparent", borderRadius: 6, padding: "3px 8px", fontSize: 11, cursor: "pointer", color: "rgb(232,84,84)" },
      add: { border: "1px dashed rgba(128,128,128,.5)", background: "transparent", borderRadius: 6, padding: "5px 10px", fontSize: 12, cursor: "pointer", color: "inherit" },
      primary: { border: "1px solid rgba(90,140,255,.5)", background: "rgba(90,140,255,.18)", borderRadius: 6, padding: "5px 14px", fontSize: 12, cursor: "pointer", color: "inherit", fontWeight: 600 },
      status: { fontSize: 11, opacity: 0.7, minHeight: 16 }
    };

    function rateCopy(r) {
      var s = r || {};
      return {
        cacheHit: String(s.cacheHit != null ? s.cacheHit : 0),
        cacheMiss: String(s.cacheMiss != null ? s.cacheMiss : 0),
        output: String(s.output != null ? s.output : 0)
      };
    }

    function rateToNumbers(r) {
      var out = {};
      ["cacheHit", "cacheMiss", "output"].forEach(function (k) {
        var v = Number(r && r[k]);
    ...
    // 分钟数 <-> "HH:MM"（0..1440；1440 显示为 24:00）。
    function minutesToTime(m) {
      var n = Math.round(Number(m));
    ...
    // 宿主价格表 → 可编辑草稿（模型数组 + 高峰时段，价格先以字符串保存以便自由输入）。
    function priceDraftFrom(p) {
      var src = (p && p.models) || {};
      var models = Object.keys(src).map(function (k) {
        var row = src[k] || {};
        return { key: k, label: String(row.label || ""), offPeak: rateCopy(row.offPeak), peak: rateCopy(row.peak) };
      });
      var winSrc = Array.isArray(p && p.peakWindows) ? p.peakWindows : DEFAULT_PRICING.peakWindows;
      var peakWindows = winSrc.map(function (w) {
        return { start: minutesToTime(w && w.start), end: minutesToTime(w && w.end) };
      });
      return { weekendOffPeak: !(p && p.weekendOffPeak === false), peakWindows: peakWindows, models: models };
    }
    ...
    function PriceEditor(props) {
      ...
      useEffect(function () {
        var alive = true;
        getPricing().then(function (p) { if (alive) setDraft(priceDraftFrom(p)); });
        return function () { alive = false; };
      }, []);

      if (!draft) return el("div", { style: stPrice.box }, el("div", { style: stSet.note }, t.loadingPrices));

      function patchDraft(patch) {
        setDraft(function (d) { return Object.assign({}, d, patch); });
      }
      function patchModel(index, patch) {
        setDraft(function (d) {
          var models = d.models.slice();
          models[index] = Object.assign({}, models[index], patch);
          return Object.assign({}, d, { models: models });
        });
      }
      function patchRate(index, period, field, value) {
        setDraft(function (d) {
          var models = d.models.slice();
          var row = Object.assign({}, models[index]);
          var rates = Object.assign({}, row[period]);
          rates[field] = value;
          row[period] = rates;
          models[index] = row;
          return Object.assign({}, d, { models: models });
      ...
      function addModel() {
        setDraft(function (d) {
          return Object.assign({}, d, {
            models: d.models.concat([{ key: "", label: "", offPeak: rateCopy(null), peak: rateCopy(null) }])
          });
        });
      }
      ...
      function patchWindow(index, field, value) {
        setDraft(function (d) {
          var windows = d.peakWindows.slice();
          var row = Object.assign({}, windows[index]);
          row[field] = value;
          windows[index] = row;
          return Object.assign({}, d, { peakWindows: windows });
        });
      }
      function addWindow() {
        setDraft(function (d) {
          return Object.assign({}, d, { peakWindows: d.peakWindows.concat([{ start: "09:00", end: "12:00" }]) });
        });
      }
      ...
      function save() {
        ...
        draft.models.forEach(function (row) {
          ...
          if (!key) { err = t.modelKeyRequired; return; }
          if (models[key]) { err = t.modelKeyDup + "：" + key; return; }
          models[key] = {
            label: String(row.label || key).trim() || key,
            offPeak: rateToNumbers(row.offPeak),
            peak: rateToNumbers(row.peak)
          };
        });
        if (err) { setStatus(err); return; }
        if (Object.keys(models).length === 0) { setStatus(t.emptyModels); return; }
        var windows = [];
        draft.peakWindows.forEach(function (w) {
          var s = timeToMinutes(w.start);
          var e = timeToMinutes(w.end);
          if (!Number.isFinite(s) || !Number.isFinite(e) || s < 0 || e > 1440 || s >= e) { err = t.windowInvalid; return; }
        ...
        api({
          action: "setPrices",
          prices: { weekendOffPeak: !!draft.weekendOffPeak, peakWindows: windows, replaceModels: true, models: models }
        }).then(function (res) {
          if (res && res.ok && res.pricing) {
            applyPricingResult(res.pricing);
            setDraft(priceDraftFrom(res.pricing));
            setStatus(t.savedPrices + " " + t.footerRefreshed);
          } else {
            setStatus((res && res.error) ? res.error : t.priceError);
          }
        }).catch(function () { setStatus(t.priceError); }).then(function () { setBusy(false); });
      }
      function resetPrices() {
        setBusy(true);
        setStatus(t.saving);
        api({ action: "resetPrices" }).then(function (res) {
          if (res && res.ok && res.pricing) {
            applyPricingResult(res.pricing);
            setDraft(priceDraftFrom(res.pricing));
            setStatus(t.savedPrices);
          } else {
            setStatus((res && res.error) ? res.error : t.priceError);
          }
        }).catch(function () { setStatus(t.priceError); }).then(function () { setBusy(false); });
      ...
      function rateRow(model, index, period, title) {
        return el("div", { style: stPrice.grid },
          el("span", { style: stPrice.th }, title),
          el("input", { type: "number", min: "0", step: "0.01", style: stPrice.num, value: model[period].cacheHit, onChange: function (e) { patchRate(index, period, "cacheHit", e.target.value); } }),
          el("input", { type: "number", min: "0", step: "0.01", style: stPrice.num, value: model[period].cacheMiss, onChange: function (e) { patchRate(index, period, "cacheMiss", e.target.value); } }),
          el("input", { type: "number", min: "0", step: "0.01", style: stPrice.num, value: model[period].output, onChange: function (e) { patchRate(index, period, "output", e.target.value); } })
        );
      }

      return el("div", { style: stPrice.box },
        el("div", { style: stSet.row },
          el("span", { style: stSet.label }, t.modelsTitle),
          el("span", { style: stPrice.th }, t.rateUnit)
        ),
        el("div", { style: stSet.note }, t.modelsHint),
        draft.models.map(function (model, index) {
          return el("div", { key: index, style: stPrice.model },
            el("div", { style: stPrice.line },
              el("input", { style: stPrice.keyInput, placeholder: t.colKey, value: model.key, onChange: function (e) { patchModel(index, { key: e.target.value }); } }),
              el("input", { style: stPrice.labelInput, placeholder: t.colLabel, value: model.label, onChange: function (e) { patchModel(index, { label: e.target.value }); } }),
              el("button", { type: "button", style: stPrice.del, onClick: function () { removeModel(index); } }, t.removeModel)
            ),
            el("div", { style: stPrice.grid },
              el("span", {}),
              el("span", { style: stPrice.th }, t.rateHit),
              el("span", { style: stPrice.th }, t.rateMiss),
              el("span", { style: stPrice.th }, t.rateOut)
            ),
            rateRow(model, index, "peak", t.colPeak),
            rateRow(model, index, "offPeak", t.colOffPeak)
          );
        }),
        el("div", { style: stSet.row },
          el("span", { style: stSet.label }, t.peakWindowsTitle)
        ),
        el("div", { style: stSet.note }, t.peakWindowsHint),
        draft.peakWindows.map(function (w, index) {
          return el("div", { key: index, style: stPrice.line },
            el("input", { type: "time", step: "60", style: stPrice.time, value: w.start, onChange: function (e) { patchWindow(index, "start", e.target.value); } }),
            el("span", {}, "–"),
            el("input", { type: "time", step: "60", style: stPrice.time, value: w.end, onChange: function (e) { patchWindow(index, "end", e.target.value); } }),
            el("button", { type: "button", style: stPrice.del, onClick: function () { removeWindow(index); } }, t.removeModel)
          );
        }),
        el("div", { style: stPrice.line },
          el("button", { type: "button", style: stPrice.add, onClick: addWindow }, t.addWindow)
        ),
        el("div", { style: stPrice.line },
          el("button", { type: "button", style: stPrice.add, onClick: addModel }, t.addModel),
          el("label", { style: stPrice.line },
            el("input", { type: "checkbox", checked: !!draft.weekendOffPeak, onChange: function (e) { patchDraft({ weekendOffPeak: e.target.checked }); } }),
            el("span", { style: stSet.note }, t.weekendLabel)
          )
        ),
        el("div", { style: stPrice.line },
          el("button", { type: "button", style: busy ? Object.assign({}, stPrice.primary, stSet.btnBusy) : stPrice.primary, disabled: busy, onClick: save }, t.savePrices),
          el("button", { type: "button", style: busy ? Object.assign({}, stSet.btn, stSet.btnBusy) : stSet.btn, disabled: busy, onClick: resetPrices }, t.resetPrices)
        ),
        status ? el("div", { style: stPrice.status }, status) : null
      );
    }
    ...
    function apply(ctx) {
      ...
      var conversationEvents = ctx.get("uiConversation") && ctx.get("uiConversation").events;
