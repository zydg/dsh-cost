 ...
 *  4. persists records to <workspace>/dsh-cost/usage-records.json and
 *     price overrides to <workspace>/dsh-cost/pricing.json;
...
export const DATA_FILE = 'usage-records.json'
export const PRICING_FILE = 'pricing.json'
export const MAX_RECORDS = 200000
export const BALANCE_FILE = 'balance-history.json'
...
export default {
  ...
  apply(ctx) {
    ...
    try {
      ...
      function persistBalanceNow() {
        if (!fs || !balancePath || !persistOk) return Promise.resolve()
        const text = JSON.stringify(balanceHistory)
        const policy = sessionPolicy()
        writeChain = writeChain
          .then(() => fs.resolve(balancePath))
          .then((target) => fs.writeText(target, text, undefined, undefined, policy || undefined))
          .catch(() => {})
        return writeChain
      }
      ...
      function persistPricing() {
        ...
        return fs.resolve(pricingPath)
          .then((target) => fs.writeText(target, JSON.stringify(pricing), undefined, undefined, policy || undefined))
          .catch(() => {})
      }
      ...
      async function loadPricing(policy) {
        ...
        try {
          ...
          if (typeof data.effectiveAt === 'string') pricing.effectiveAt = data.effectiveAt
          if (Array.isArray(data.peakWindows)) pricing.peakWindows = data.peakWindows
          const src = data.models
      ...
      async function initBalanceFile(policy) {
        if (!fs || !balancePath) return
        try {
          const target = await fs.resolve(balancePath)
        ...
        try {
          const target = await fs.resolve(balancePath)
          await fs.writeText(target, JSON.stringify(balanceHistory), undefined, undefined, policy || undefined)
        } catch {}
      }
      ...
      async function migrateLegacyFiles() {
        ...
        for (const dir of legacyDirs) {
          ...
          await mergeRecordsFile(joinPath(dir, DATA_DIR, DATA_FILE))
          await mergeBalanceFile(joinPath(dir, DATA_DIR, BALANCE_FILE))
        }
      }
      ...
      async function tryInitWithRoot(candidate, policy) {
        ...
        try {
          ...
          balancePath = joinPath(path.dirname(tryPath), BALANCE_FILE)
          await initBalanceFile(policy)
          root = normPath(candidate)
          dataPath = tryPath
          pricingPath = joinPath(path.dirname(dataPath), PRICING_FILE)
